...
void InternalSocket::AuthHandlers::Setup(MessageHandler* messageHandler)
{
    messageHandler->SetMessageHandler(Opcode::SMSG_LOGON_CHALLENGE, InternalSocket::AuthHandlers::HandshakeHandler);
    messageHandler->SetMessageHandler(Opcode::SMSG_LOGON_HANDSHAKE, InternalSocket::AuthHandlers::HandshakeResponseHandler);
}
bool InternalSocket::AuthHandlers::HandshakeHandler(std::shared_ptr<NetworkClient> networkClient, NetworkPacket* packet)
{
    if (networkClient->GetStatus() != ConnectionStatus::AUTH_CHALLENGE)
        return false;
...
bool InternalSocket::AuthHandlers::HandshakeResponseHandler(std::shared_ptr<NetworkClient> networkClient, NetworkPacket* packet)
{
    if (networkClient->GetStatus() != ConnectionStatus::AUTH_HANDSHAKE)
        return false;
